...
// Iconos SVG personalizados (se mantienen igual)
const CodeIcon = () => (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                    {services.map((service, index) => (
                        <div
                            ...
                            // Usamos group/transform/translate-y solo en lg: (escritorio)
                            className={`relative group rounded-xl cursor-pointer 
                                transition-all duration-500 
                                lg:hover:-translate-y-2
                            `}
                        >
                            {/* Borde brillante animado - AHORA CON CLASES CONDICIONALES PARA MANTENER EL EFECTO EN HOVER SOLO EN ESCRITORIO */}
                            <div 
                                className={`absolute -inset-px rounded-xl bg-linear-to-r from-[#00fd9c] to-[#00fdc8] blur-sm transition-all duration-500 
                                // Ocultar por defecto, mostrar solo en hover Y en pantalla grande (lg:)
                                ${hoveredIndex === index ? 'lg:opacity-70 lg:animate-pulseGlow' : 'opacity-0'}
                                hidden lg:block // Aseguramos que solo se muestre en escritorio
                                `}
                            ></div>

                            {/* Contenedor principal */}
                            <div className="relative bg-[#232229] rounded-xl shadow-lg transition-all duration-500 overflow-hidden">
                                ...
                                <div className="relative w-full h-48 lg:h-80 overflow-hidden">
                                    <img 
                                        ...
                                        className={`w-full h-full object-cover transition-all duration-500 
                                            brightness-75 lg:group-hover:brightness-50 lg:group-hover:scale-110
                                        `}
                                    />
                                </div>

                                {/* Contenido completo (Glassmorph en móvil, Hover en desktop) */}
                                <div
                                    className={`relative lg:absolute inset-0 bg-[#232229] p-6 lg:p-8 flex flex-col justify-between 
                                        rounded-xl transition-opacity duration-700 
                                        
                                        // Comportamiento en Desktop (Hover)
                                        lg:bg-[#232229]/95 
                                        lg:opacity-0 lg:group-hover:opacity-100 lg:pointer-events-auto
                                    `}
                                >
                                        ...
                                        <div className="flex justify-between items-start mb-6">
                                            ...
                                            <span 
                                                className={`text-[#00fd9c] text-3xl transition-transform duration-300 
                                                    lg:group-hover:rotate-12 lg:group-hover:scale-110
                                                `}
                                            >
                                                {service.icon}
                                            </span>
